...
func TestSentinelReplicaRestartLoop(t *testing.T) {
	cfg := chaosmetrics.LoadConfig(t)
	chaosmetrics.SkipUnless(t, cfg, "replica-restart-loop")

	s := suite.New(t)

	builder := cluster.NewBuilder(s.Harness.Namespace(), s.Harness.RedisImage()).
		WithName(fmt.Sprintf("crl-%s", uniqueSuffix())).
		WithLabels(map[string]string{"suite": "chaos", "scenario": "replica-restart-loop"}).
	...
	scenarioMetrics := chaosmetrics.NewScenario(t, s.Harness, cr, cfg, "replica-restart-loop")
	...
	s.Step("restart-loop", func(ctx context.Context) {
		...
		scenarioMetrics.RecordDowntime("replica-restart-loop", downtime)
	})
}
